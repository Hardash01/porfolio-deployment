            ...
            <div className="max-w-screen-xl mx-auto md:w-5/6 lg:w-4/6">
                <div className="flex flex-col md:flex-row justify-center items-center md:items-start md:justify-between mx-3 md:mx-0 py-10 font-body">
                    <div className="flex flex-col items-center md:items-start md:text-left justify-center mb-5 md:mb-0 text-center md:w-1/3 [text-wrap:balance]">
                        ...
                        <p className="text-white text-sm">Transformando ideas en realidad a traves de codigo limpio y soluciones innovadoras</p>
                    ...
                    <div className="mb-3 md:mb-0">
                        ...
                        <ul className="text-sm flex flex-col text-center md:text-left gap-4 justify-between">
                            {enlacesData.map(enlace => (
                                <li key={enlace.id}>
                                    <a href={enlace.link} className="pb-0.5 border-b-2 border-transparent text-white transition-all duration-500 hover:border-naranja">
                                        {enlace.name}
                                    </a>
                                </li>
                            ))}
                    ...
                    <div className="flex items-center flex-col mt-3 md:mt-0">
                        ...
                        <p className="text-white text-sm mb-3">Creado con:</p>
                        <ul className="flex gap-3 md:gap-0 md:flex-col mb-3 space-y-1">
                            {webAbout.map(web => (
                                <li key={web.id} className="flex items-center gap-1  md:gap-3">
                                    ...
                                    <p className="text-white text-sm">{web.name}</p>
                                </li>
                            ))}
                        </ul>
                        <p className="text-white text-sm">por <span className="font-semibold text-sm hover:text-naranja">Edwin Santos</span></p>
                    </div>
                </div>
            </div>
        </div>
